...
The repository ships the core pipeline and figures for Parts 2 and 6, and a strong baseline for Part 3. For completeness, the full plan is in `ExpPlan.md`.
...
- Part 6 (Verification): included conceptually — saboteur‑only run demonstrates fidelity degradation and learning. A dedicated `verify_saboteur` script can be added or adapted if you want a one‑shot check on a perfect GHZ circuit.
